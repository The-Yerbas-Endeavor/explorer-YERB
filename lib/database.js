  ...
  save_masternode: function (raw_masternode, cb) {
    ...
    module.exports.find_masternode((raw_masternode.proTxHash != null ? raw_masternode.proTxHash : raw_masternode.txhash), function (masternode) {
      ...
      if (settings.claim_address_page.enabled == true) {
        ...
        var state = raw_masternode.state || {};

   var claimAddress =
    state.payoutAddress ||
    raw_masternode.payoutAddress ||
    raw_masternode.payee ||
    raw_masternode.addr ||
    '';

find_address(claimAddress, false, function(address) {
          if (address) {
  ...
  add_update_masternode: function(masternode, add, cb) {
    var isDeterministic =
      typeof masternode.proTxHash === 'string' &&
      masternode.proTxHash.trim() !== '';

    var txhash = isDeterministic
  ? (
...
var proTxHash = isDeterministic
  ...
  : '';
    ...
    if (isDeterministic) {
      var state = masternode.state || {};
        ...
        mn = new Masternode({
          ...
          outidx:
            masternode.collateralIndex != null
              ? Number(masternode.collateralIndex)
              : Number(masternode.outidx || 0),
          ...
          ip_address: serviceAddress,

          registeredHeight:
            Number(
              masternode.registeredHeight ||
              masternode.registeredheight ||
              state.registeredHeight ||
              state.registeredheight ||
              0
            ),
          ...
          last_paid_block:
            Number(
              masternode.lastPaidHeight ||
              masternode.lastpaidblock ||
              state.lastPaidHeight ||
              state.lastpaidblock ||
              0
            ),
        ...
        activetime:
          Number(masternode.activetime || 0),
        ...
        poseRevivedHeight:
          Number(
            state.PoSeRevivedHeight ??
            masternode.PoSeRevivedHeight ??
            masternode.poseRevivedHeight ??
            -1
          ),
        ...
        rank: Number(masternode.rank || 0),
        network: masternode.network || '',
        version: Number(masternode.version || 0),

        claim_name:
          masternode.claim_name == null
            ? ''
            : masternode.claim_name
      });
    } else {
      mn = new Masternode({
    ...
    var updateData = mn.toObject();

    delete updateData._id;
    delete updateData.__v;

    Masternode.updateOne(
      {
        txhash: txhash,
        outidx: Number(updateData.outidx || 0)
      },
      {
        $set: updateData
      },
